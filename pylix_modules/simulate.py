...
def refine_multi_variable(xtal, basis, cell, hkl, bloch, cbed,
    ...
    if rc.image_processing != 0:
        sim = gaussian_filter(sim, sigma=(rc.blur_radius, rc.blur_radius, 0))
    ...
    if np.sum(abs(cbed.lacbed_sig[j])) < eps:
        ...
        # masked figures of merit
        masked_sim = (cbed.lacbed_mask[j] * sim)
        masked_expt = (cbed.lacbed_mask[j] * cbed.lacbed_expt)
        masked_ref = (cbed.lacbed_mask[j] * cbed.lacbed_ref)
        refined_variable_hkl_fom = np.empty(rc.n_out)
        for i in range(rc.n_out):
            ex = masked_expt[:, :, i]
            ex = ex[ex != 0]  # 1d masked experimental image without zeros
            ex = (ex - ex.mean()) / ex.std()
            re = masked_ref[:, :, i]
            re = re[re != 0]  # 1d masked simulated image without zeros
            re = (re - re.mean()) / re.std()
            si = masked_ref[:, :, i]
            si = si[si != 0]  # 1d masked simulated image without zeros
            si = (si - si.mean()) / si.std()
            # individual figure of merit for lacbed pattern hkl
            # NB as usual we have best fom = 0
            refined_variable_hkl_fom[i] = 1 - np.mean(ex * re)
        rc.refined_variable_fom[j] = np.mean(refined_variable_hkl_fom)
...
def figure_of_merit(bloch, cbed, rc, k):
    ...
    for i in range(rc.n_thickness):
        ...
        # apply the blur
        if rc.image_processing != 0:
            cbed.lacbed_sim[i] = gaussian_filter(cbed.lacbed_sim[i],
        ...
        if rc.correlation_type == 0:
            ...
            for ind in range(rc.n_out):
                ...
                m_i = cbed.lacbed_mask_i[0, :, :, ind]
                masked_expt_i = e0[m_i > np.max(m_i)/2]
                masked_sim_i = s0[m_i > np.max(m_i)/2]
                # normalisation
                n = len(masked_expt_i)
                mean_expt_i = np.mean(masked_expt_i)
                std_expt_i = np.std(masked_expt_i)
                mean_sim_i = np.mean(masked_sim_i)
                std_sim_i = np.std(masked_sim_i)
                masked_expt_i = (masked_expt_i - mean_expt_i) / std_expt_i
                masked_sim_i = (masked_sim_i - mean_sim_i) / std_sim_i
                z_i = np.sum(masked_expt_i*masked_sim_i / n)

                # m_j = cbed.lacbed_mask_j[0, :, :, ind]
                # masked_expt_j = e0[m_j > np.max(m_j)/2]
                # masked_sim_j = s0[m_j > np.max(m_j)/2]
                # n = len(masked_expt_j)
                # mean_expt_j = np.mean(masked_expt_j)
                # std_expt_j = np.std(masked_expt_j)
                # mean_sim_j = np.mean(masked_sim_j)
                # std_sim_j = np.std(masked_sim_j)
                # masked_expt_j = (masked_expt_j - mean_expt_j) / std_expt_j
                # masked_sim_j = (masked_sim_j - mean_sim_j) / std_sim_j
                # z_j = np.sum(masked_expt_j*masked_sim_j / n)
                
                fom_array[i, ind] = 1 - z_i  # +z_j)/2
